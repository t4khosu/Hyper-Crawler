...
class Graph(object):
    ...
    def load(self, source, minRefs=2, maxRefs=-1):
        ...
        for key in self.mapping:
            if len(self.mapping[key]) >= minRefs and (
                    len(self.mapping[key]) <= maxRefs or maxRefs == -1
            ):
    ...
    def draw(self, title):
        ...
        nx.draw_networkx_nodes(
            g, pos, node_size=self.node_sizes, node_color=self.color_map, alpha=0.4
        )
